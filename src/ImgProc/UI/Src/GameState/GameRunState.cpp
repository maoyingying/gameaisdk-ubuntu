...
void CGameRunState::Handle(const tagFrameContext &stFrameCtx, CContext *pContext)
{
    ...
    {
        LOGD("match run state: framecnt %d, over check interval is %d, skip frame", nFrameCnt, nOverCheckInterval);
        return;
    }
    ...
    if (nIndex > -1)
    {
        ...
        bool bRst = CAction::getInstance()->DoAction(stFrameCtx, stRst, uiState, GAME_REG_STATE_OVER);
        if (bRst)
        {
    ...
    {
        // send none action
        LOGD("run state: frame count is %d, interval is %d, skip frame", nFrameCnt, nPOPCheckInterval);
        CAction::getInstance()->DoAction(stFrameCtx, stRst, uiState, GAME_REG_STATE_RUN);
        return;
    }
    ...
    if (nIndex > -1)
    {
        ...
        CAction::getInstance()->SendAction()->SendClickAction(stFrameCtx, stPOPUIState, stPOPUIActionState);
        return;
    }
...
// overload to return state
int CGameRunState::Handle(const tagFrameContext &stFrameCtx, CContext *pContext, int getState)
{
    // get "CheckInterval" value from game over config
    int nOverCheckInterval = CGameOverCfg::getInstance()->GetCheckInterval();
    int nFrameCnt          = stFrameCtx.nFrameCount;
    ...
    if (nIndex > -1)
    {
        ...
        // return;
        if (isGameOver) return 1;   // 1 denotes gameover
        return 2;                   // 2 denotes others
    }
