...
void CHallState::Handle(const tagFrameContext &stFrameCtx, CContext *pContext)
{
    ...
    if (!uiArray.empty())
    {   // detect hall, if matched, result nIndex is the UID of Hall
    ...
    if (nIndex > -1)
    {
        bool bRst = CAction::getInstance()->DoAction(stFrameCtx, stUIRegRst, uiArray, GAME_REG_STATE_START);
        if (bRst)
        {
...
int CHallState::Handle(const tagFrameContext &stFrameCtx, CContext *pContext, int getState)
{
    tagUIRegResult stUIRegRst;
    UIStateArray   uiArray;
